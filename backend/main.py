"""
MAIN BACKEND SERVER - COMPLETE WITH DRIVER DOCUMENTS, SMS, PAYMENTS
"""
...
async def verify_otp_code(phone: str, code: str) -> bool:
    conn = await get_db()
    result = await conn.fetchrow("SELECT * FROM otps WHERE phone = $1 AND code = $2 AND expires_at > $3", phone, code, datetime.now())
    if result:
        await conn.execute("DELETE FROM otps WHERE phone = $1", phone)
        await release_db(conn)
        return True
...
async def create_user(phone: str, name: str, role: str):
    ...
    await conn.execute('INSERT INTO users (id, phone, name, role, is_verified, referral_code) VALUES ($1, $2, $3, $4, 0, $5)', user_id, phone, name, role, referral_code)
...
async def update_driver_rating(driver_id: str, new_rating: int):
    ...
    if user:
        ...
        await conn.execute("UPDATE drivers SET rating = $1, total_rides = total_rides + 1 WHERE user_id = $2", round(new_avg, 1), driver_id)
    await release_db(conn)
...
async def update_driver_location(user_id: str, lat: float, lng: float, is_online: bool = True):
    ...
    if not driver:
        ...
        await conn.execute('INSERT INTO drivers (id, user_id, is_approved, is_online, current_lat, current_lng) VALUES ($1, $2, 1, $3, $4, $5)', driver_id, user_id, 1 if is_online else 0, lat, lng)
    else:
        await conn.execute('UPDATE drivers SET current_lat = $1, current_lng = $2, is_online = $3 WHERE user_id = $4', lat, lng, 1 if is_online else 0, user_id)
    await release_db(conn)
...
async def save_ride(ride_data: dict):
    ...
    await conn.execute('''
        INSERT INTO rides (id, customer_id, driver_id, pickup_lat, pickup_lng,
    ...
    await conn.execute('UPDATE drivers SET total_earnings = total_earnings + $1, rides_today = rides_today + 1 WHERE user_id = $2', ride_data.get('fare', 0), driver_id)
...
async def update_wallet_balance(user_id: str, amount: int, transaction_type: str = 'credit') -> bool:
    ...
    if transaction_type == 'credit':
        result = await conn.execute('UPDATE wallets SET balance = balance + $1, total_deposited = total_deposited + $1, updated_at = CURRENT_TIMESTAMP WHERE user_id = $2', amount, user_id)
    else:
        result = await conn.execute('UPDATE wallets SET balance = balance - $1, total_withdrawn = total_withdrawn + $1, updated_at = CURRENT_TIMESTAMP WHERE user_id = $2 AND balance >= $1', amount, user_id)
    await release_db(conn)
...
async def create_transaction(transaction_data: dict) -> dict:
    ...
    await conn.execute('''
        INSERT INTO transactions (id, user_id, ride_id, amount, type, method, status, reference, provider_reference, description)
...
async def get_transactions(user_id: str, limit: int = 50) -> list:
    conn = await get_db()
    transactions = await conn.fetch('SELECT * FROM transactions WHERE user_id = $1 ORDER BY created_at DESC LIMIT $2', user_id, limit)
    await release_db(conn)
...
async def get_driver_document_status(driver_id: str):
    conn = await get_db()
    result = await conn.fetchrow("SELECT status, rejection_reason, submitted_at FROM driver_documents WHERE driver_id = $1", driver_id)
    await release_db(conn)
    return result

async def approve_driver_documents(driver_id: str, rejection_reason: str = None):
    conn = await get_db()
    if rejection_reason:
        await conn.execute("UPDATE driver_documents SET status = 'rejected', reviewed_at = CURRENT_TIMESTAMP, rejection_reason = $2 WHERE driver_id = $1", driver_id, rejection_reason)
        await conn.execute("UPDATE drivers SET is_approved = 0 WHERE user_id = $1", driver_id)
    else:
        await conn.execute("UPDATE driver_documents SET status = 'approved', reviewed_at = CURRENT_TIMESTAMP WHERE driver_id = $1", driver_id)
        await conn.execute("UPDATE drivers SET is_approved = 1 WHERE user_id = $1", driver_id)
    await release_db(conn)
...
async def update_driver_approval_status(driver_id: str, is_approved: bool):
    conn = await get_db()
    await conn.execute("UPDATE drivers SET is_approved = $1 WHERE user_id = $2", 1 if is_approved else 0, driver_id)
    await release_db(conn)
...
async def get_driver_analytics(driver_id: str):
    ...
    result = await conn.fetchrow('''
        SELECT COUNT(*) as total_rides, AVG(customer_rating) as avg_rating,
    ...
    ''', driver_id)
    weekly_rides = await conn.fetchval('SELECT COUNT(*) FROM rides WHERE driver_id = $1 AND status = "completed" AND requested_at > NOW() - INTERVAL \'7 days\'', driver_id)
    peak = await conn.fetchrow('SELECT EXTRACT(HOUR FROM requested_at) as hour, COUNT(*) FROM rides WHERE driver_id = $1 GROUP BY hour ORDER BY COUNT(*) DESC LIMIT 1', driver_id)
    await release_db(conn)
    peak_hour = 17
...
async def create_referral(referrer_id: str, referred_id: str, referrer_name: str = None, referred_name: str = None):
    ...
    await conn.execute('INSERT INTO referrals (id, referrer_id, referred_id, referrer_name, referred_name, status) VALUES ($1, $2, $3, $4, $5, \'pending\')', referral_id, referrer_id, referred_id, referrer_name, referred_name)
    await release_db(conn)
    return referral_id
...
async def complete_referral(referral_id: str):
    ...
    if referral:
        ...
        await conn.execute('UPDATE referrals SET status = \'completed\', reward = $1, completed_at = CURRENT_TIMESTAMP WHERE id = $2', BONUS_AMOUNT, referral_id)
    await release_db(conn)
...
async def save_support_message(user_id: str, message: str, from_admin: bool = False):
    conn = await get_db()
    await conn.execute('INSERT INTO support_messages (user_id, message, from_admin) VALUES ($1, $2, $3)', user_id, message, 1 if from_admin else 0)
    await release_db(conn)

async def get_support_messages(user_id: str, limit: int = 50):
    conn = await get_db()
    messages = await conn.fetch('SELECT * FROM support_messages WHERE user_id = $1 ORDER BY created_at ASC LIMIT $2', user_id, limit)
    await release_db(conn)
...
async def save_feedback(user_id: str, feedback: str, rating: int = 0):
    conn = await get_db()
    await conn.execute('INSERT INTO feedback (user_id, feedback, rating) VALUES ($1, $2, $3)', user_id, feedback, rating)
    await release_db(conn)
...
@app.post("/api/driver/upload-documents")
async def upload_driver_documents(
    driver_id: str = Form(...),
    license_photo: UploadFile = File(...),
    bike_registration: UploadFile = File(...),
    profile_photo: Optional[UploadFile] = File(None)
):
    try:
        driver_folder = os.path.join(UPLOAD_DIR, driver_id)
        os.makedirs(driver_folder, exist_ok=True)
        
        license_filename = f"license_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
        license_path = os.path.join(driver_folder, license_filename)
        with open(license_path, "wb") as f:
            shutil.copyfileobj(license_photo.file, f)
        
        bike_filename = f"bike_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
        bike_path = os.path.join(driver_folder, bike_filename)
        with open(bike_path, "wb") as f:
            shutil.copyfileobj(bike_registration.file, f)
        
        profile_path = None
        if profile_photo:
            profile_filename = f"profile_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
            profile_path = os.path.join(driver_folder, profile_filename)
            with open(profile_path, "wb") as f:
                shutil.copyfileobj(profile_photo.file, f)
        
        conn = await get_db()
        existing = await conn.fetchrow("SELECT id FROM driver_documents WHERE driver_id = $1", driver_id)
        if existing:
            await conn.execute("""
                UPDATE driver_documents 
                SET license_photo = $1, bike_registration_photo = $2, profile_photo = $3, 
                    status = 'pending', submitted_at = CURRENT_TIMESTAMP, rejection_reason = NULL
                WHERE driver_id = $4
            """, license_path, bike_path, profile_path, driver_id)
        else:
            await conn.execute("""
                INSERT INTO driver_documents (driver_id, license_photo, bike_registration_photo, profile_photo, status)
                VALUES ($1, $2, $3, $4, 'pending')
            """, driver_id, license_path, bike_path, profile_path)
        await release_db(conn)
        return {"success": True, "message": "Documents uploaded successfully. Awaiting approval."}
    except Exception as e:
        print(f"❌ Upload error: {e}")
        return {"success": False, "message": str(e)}

@app.get("/api/driver/document-status/{driver_id}")
async def get_driver_document_status_endpoint(driver_id: str):
    result = await get_driver_document_status(driver_id)
    if result:
        return {
            "success": True,
...
@app.post("/api/admin/verify-driver/{driver_id}")
async def verify_driver(driver_id: str, request: dict):
    action = request.get('action')
    reason = request.get('reason', '')
    if action == 'approve':
        await approve_driver_documents(driver_id)
        return {"success": True, "message": "Driver approved successfully"}
    else:
        await approve_driver_documents(driver_id, reason)
        return {"success": True, "message": "Driver rejected"}

@app.post("/api/feedback")
async def submit_feedback(request: dict):
    await save_feedback(request.get('user_id'), request.get('feedback'), request.get('rating', 0))
    return {"success": True}
...
async def get_wallet(user_id: str):
    try:
        ...
        return {"success": True, "balance": wallet['balance'], "total_deposited": wallet['total_deposited'], "total_withdrawn": wallet['total_withdrawn']}
    except Exception as e:
        return {"success": False, "message": str(e)}
...
async def deposit_to_wallet(request: dict):
    ...
    await create_transaction({'user_id': user_id, 'amount': amount, 'type': 'deposit', 'method': method, 'status': 'completed', 'reference': reference, 'description': f'Deposit of UGX {amount} via {method.upper()}'})
    await update_wallet_balance(user_id, amount, 'credit')
    return {"success": True, "message": f"Successfully deposited UGX {amount}", "reference": reference}
...
async def process_ride_payment(request: dict):
    ...
    if method == 'wallet':
        ...
        await create_transaction({'user_id': user_id, 'ride_id': ride_id, 'amount': amount, 'type': 'payment', 'method': 'wallet', 'status': 'completed', 'reference': reference, 'description': f'Payment for ride {ride_id} from wallet'})
        return {"success": True, "message": f"Payment of UGX {amount} completed from wallet", "reference": reference}
    elif method == 'cash':
        await create_transaction({'user_id': user_id, 'ride_id': ride_id, 'amount': amount, 'type': 'payment', 'method': 'cash', 'status': 'completed', 'reference': reference, 'description': f'Cash payment for ride {ride_id} - paid directly to driver'})
        ...
        await create_transaction({'user_id': user_id, 'ride_id': ride_id, 'amount': amount, 'type': 'payment', 'method': method, 'status': 'completed', 'reference': reference, 'description': f'Payment for ride {ride_id} via {method.upper()}'})
...
async def driver_withdrawal(request: dict):
    ...
    await create_transaction({'user_id': driver_id, 'amount': amount, 'type': 'withdrawal', 'method': method, 'status': 'pending', 'reference': reference, 'description': f'Withdrawal request to {method.upper()} {phone}'})
    new_balance = await get_wallet_balance(driver_id)
    return {"success": True, "message": f"Withdrawal request for UGX {amount} submitted", "reference": reference, "new_balance": new_balance}
...
async def support_ws(user_id: str, websocket: WebSocket):
    ...
    for msg in messages:
        await websocket.send_json({'type': 'message', 'message': msg['message'], 'from_admin': bool(msg['from_admin']), 'timestamp': msg['created_at']})
    try:
        while True:
            ...
            if message:
                ...
                await websocket.send_json({'type': 'message', 'message': message, 'from_admin': False, 'timestamp': datetime.now().isoformat()})
                if 'admin' in manager.active_connections:
                    await manager.send('admin', {'type': 'new_message', 'user_id': user_id, 'message': message, 'timestamp': datetime.now().isoformat()})
            ...
            if msg_type == 'location_update':
                ...
                for ride_id, ride in active_rides.items():
                    if ride['driver_id'] == driver_id:
                        await manager.send(ride['customer_id'], {'type': 'driver_location_update', 'lat': data['lat'], 'lng': data['lng']})
                ...
                if ride_id in pending_rides:
                    ...
                    active_rides[ride_id] = {'driver_id': driver_id, 'customer_id': customer_id, 'distance_km': distance, 'fare': fare, 'payment_method': payment_method}
                    await manager.send(customer_id, {'type': 'driver_assigned', 'driver_id': driver_id, 'ride_id': ride_id, 'fare': fare, 'distance_km': distance, 'driver_location': driver_locations.get(driver_id, {'lat': 0, 'lng': 0})})
                    await websocket.send_json({'type': 'ride_accepted', 'fare': fare, 'distance_km': distance, 'message': f'Ride accepted! Fare: UGX {fare:,}'})
                ...
                if ride_id in active_rides:
                    ...
                    await manager.send(customer_id, {'type': 'ride_started', 'message': 'Your ride has started!'})
                ...
                if ride_id in active_rides:
                    ...
                    if payment_method == 'wallet':
                        ...
                        if customer_balance < fare:
                            await websocket.send_json({'type': 'payment_error', 'message': 'Insufficient wallet balance'})
                            await manager.send(customer_id, {'type': 'payment_error', 'message': 'Insufficient wallet balance'})
                            del active_rides[ride_id]
                            return
                    ride_data = {'id': ride_id, 'customer_id': customer_id, 'driver_id': driver_id, 'distance_km': distance, 'fare': fare, 'surge_multiplier': 1.0, 'status': 'completed', 'payment_method': payment_method, 'completed_at': datetime.now()}
                    ...
                    if payment_method == 'wallet':
                        ...
                        await websocket.send_json({'type': 'ride_completed_confirmation', 'fare': fare, 'distance_km': distance, 'message': f'Ride completed! UGX {fare:,} added to your wallet.'})
                    elif payment_method == 'cash':
                        await websocket.send_json({'type': 'ride_completed_confirmation', 'fare': fare, 'distance_km': distance, 'message': f'Ride completed! Customer will pay UGX {fare:,} in cash.'})
                    else:
                        await websocket.send_json({'type': 'ride_completed_confirmation', 'fare': fare, 'distance_km': distance, 'message': f'Ride completed! Payment processed via {payment_method.upper()}.'})
                    await manager.send(customer_id, {'type': 'ride_completed', 'fare': fare, 'distance_km': distance, 'ride_id': ride_id, 'driver_id': driver_id, 'payment_method': payment_method, 'message': f'Ride complete! Total: UGX {fare:,}'})
                    del active_rides[ride_id]
        ...
        while True:
            ...
            if msg_type == 'request_ride':
                ...
                if distance_km == 0 and 'dest_lat' in data:
                    distance_km = calculate_distance(data['pickup_lat'], data['pickup_lng'], data['dest_lat'], data['dest_lng'])
                ...
                if payment_method == 'wallet':
                    ...
                    if wallet_balance < fare:
                        await websocket.send_json({'type': 'payment_error', 'message': f'Insufficient wallet balance. Available: UGX {wallet_balance:,}, Required: UGX {fare:,}'})
                        return
                ...
                for driver_id in online_drivers:
                    ...
                    if loc:
                        ...
                        if dist < min_dist and dist <= MAX_SEARCH_RADIUS_KM:
                            ...
                            nearest = driver_id
                if nearest:
                    pending_rides[ride_id] = {'customer_id': customer_id, 'driver_id': nearest, 'pickup': data['pickup'], 'destination': data['destination'], 'pickup_lat': data['pickup_lat'], 'pickup_lng': data['pickup_lng'], 'dest_lat': data.get('dest_lat', 0), 'dest_lng': data.get('dest_lng', 0), 'distance_km': distance_km, 'fare': fare, 'payment_method': payment_method}
                    await manager.send(nearest, {'type': 'new_ride_request', 'ride_id': ride_id, 'pickup': data['pickup'], 'destination': data['destination'], 'pickup_lat': data['pickup_lat'], 'pickup_lng': data['pickup_lng'], 'dest_lat': data.get('dest_lat', 0), 'dest_lng': data.get('dest_lng', 0), 'distance_km': distance_km, 'fare': fare, 'payment_method': payment_method})
                    await websocket.send_json({'type': 'searching_for_driver', 'ride_id': ride_id, 'fare': fare, 'distance_km': distance_km, 'driver_distance_km': round(min_dist, 1), 'message': f'Searching for driver... Nearest driver is {round(min_dist, 1)}km away'})
                    print(f"   → Searching for driver within {round(min_dist, 1)}km")
                else:
                    await websocket.send_json({'type': 'no_drivers', 'message': f'No drivers within {MAX_SEARCH_RADIUS_KM}km. Please try again.'})
            elif msg_type == 'cancel_ride':
                ride_id = data['ride_id']
                if ride_id in pending_rides:
                    driver_id = pending_rides[ride_id]['driver_id']
                    await manager.send(driver_id, {'type': 'ride_cancelled', 'ride_id': ride_id})
                    del pending_rides[ride_id]
                ...
                await websocket.send_json({'type': 'rating_confirmed', 'message': f'Thank you for rating {rating} stars!'})
    except WebSocketDisconnect:
...
async def root():
    return {
        ...
        "version": "3.0.0",
        "features": ["driver_documents", "sms_otp", "surge_pricing", "referrals", "support_chat", "driver_analytics", "admin_dashboard"],
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("🚀 BODA BODA SYSTEM - PRODUCTION READY")
    print("=" * 60)
    print(f"📡 Server running on port {port}")
    print("📱 SMS OTP: Active")
    print("📄 Driver Documents: Active")
